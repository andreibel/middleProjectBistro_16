...
public class WorkerRegisterFormGUIController implements IServerResponseListener {
    ...
    public void onServerResponse(Message message) {
        ...
        else if (message.getType() == APICallType.WORKER_CREATE_ERROR){
            BistroUtilities.showMessage("Bistro Restaurant", "Due to server error, it was unable to add a new worker.");
        }
    }
    @FXML
    private void onButtonRegisterWorkerClicked(ActionEvent event) {
        if (txtFieldWorkerName.getText().isEmpty()) {
        ...
        if (!BistroUtilities.isValidFullName(txtFieldWorkerName.getText())) {
            BistroUtilities.showMessage("Bistro Restaurant", "Please enter a valid worker name");
            return;
        }
        if (!BistroUtilities.isPasswordValid(txtFieldPassword.getText())) {
            BistroUtilities.showMessage("Bistro Restaurant", "Please enter valid password");
            return;
        }
        controller.requestRegisterNewWorker(new WorkerNewRequest(txtFieldWorkerName.getText(), txtFieldPassword.getText(), null,chkBoxManager.isSelected()));
    }
    @FXML
