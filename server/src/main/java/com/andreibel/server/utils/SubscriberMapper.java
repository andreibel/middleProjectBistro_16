...
public class SubscriberMapper {
    public static Subscriber mapRelToSubscriber(ResultSet rs) throws SQLException {
        return Subscriber.builder().subscriberId(rs.getInt(Subscriber.SUBSCRIBER_ID)).email(rs.getString(Subscriber.EMAIL)).name(rs.getString(Subscriber.NAME)).phoneNumber(rs.getString(Subscriber.PHONE_NUMBER)).build();
    }
    ...
    public static Subscriber mapSubscriberNewRequestToSubscriber(SubscriberRequest subscriber) {
        return Subscriber.builder().subscriberId(-1).email(subscriber.getEmail()).name(subscriber.getName()).phoneNumber(subscriber.getPhoneNumber()).build();
    }
}
